...
if __name__ == "__main__":
    ...
    config = ast.literal_eval(
        open(model_path + "{}config".format(sys.argv[2])).readline()
    )
    ...
    fake_net.load_state_dict(
        torch.load(
            model_path + "{}model".format(sys.argv[2]), map_location=torch.device("cpu")
        )
    )  # cuda:0")))
    vit_model.load_state_dict(
        torch.load(
            model_path + "{}vitmodel".format(sys.argv[2]),
    ...
    with torch.no_grad():
        ...
        for loader_idx, item in tqdm(
            ...
            input_document = deberta_tokenizer(
                document_text, truncation=True, padding=True, return_tensors="pt"
            ).to(device)
