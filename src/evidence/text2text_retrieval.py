...
class SemanticSimilarity:
    def __init__(self, train_embeddings_file, test_embeddings_file, train_csv_path, test_csv_path, no_rerank=False):
        ...
        self.train_embeddings, self.train_ids = self._load_embeddings(train_embeddings_file)
        self.test_embeddings, self.test_ids = self._load_embeddings(test_embeddings_file)
    ...
    def search(self, query, top_k):
        ...
        hits_train = util.semantic_search(question_embedding, self.train_embeddings, top_k=top_k*5)
        hits_train = hits_train[0]  # Get the hits for the first query
        print(f"len(hits_train) = {len(hits_train)}")
        hits_test = util.semantic_search(question_embedding, self.test_embeddings, top_k=top_k*5)
        ...
        cross_inp_train = [[query, self.train_csv['evidence_enriched'][hit['corpus_id']]] for hit in hits_train]
        cross_scores_train = self.cross_encoder.predict(cross_inp_train)

        cross_inp_test = [[query, self.test_csv['evidence_enriched'][hit['corpus_id']]] for hit in hits_test]
        ...
        hits_train_cross_encoder = sorted(hits_train, key=lambda x: x.get('cross-score'), reverse=True)
        hits_train_cross_encoder = hits_train_cross_encoder[:top_k*5]
        hits_test_cross_encoder = sorted(hits_test, key=lambda x: x.get('cross-score'), reverse=True)
        ...
        for id_, score in sorted(results, key=lambda x: x[1], reverse=True):
            ...
            if len(filtered_results) == top_k:  # Stop when top_k unique scores are reached
                break
...
class TextCorpus:
    ...
    def encode_corpus(self):
        ...
        with h5py.File(h5_file_path, 'w') as h5_file:
            ...
            h5_file.create_dataset('embeddings', data=embeddings.numpy(), dtype="float16")

            h5_file.create_dataset('ids', data=[f"{self.split}_{id}" for id in ids], dtype=h5py.string_dtype())
